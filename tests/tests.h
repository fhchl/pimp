...
#define TEST_ARRAY_WITHIN(DELTA, EXPECTED, ACTUAL, NUM_ELEMENTS)                                                                                      \
    _Generic(*(EXPECTED),                                                                                                                             \
             ...
             : _test_assert_array_double_within((double)(DELTA), (double*)(EXPECTED), (double*)(ACTUAL), (NUM_ELEMENTS)),                             \
               complex float                                                                                                                          \
             : _test_assert_array_complex_float_within((float)(DELTA), (complex float*)(EXPECTED), (complex float*)(ACTUAL), (NUM_ELEMENTS)), \
               complex double                                                                                                                         \
             : _test_assert_array_complex_double_within((double)(DELTA), (complex double*)(EXPECTED), (complex double*)(ACTUAL), (NUM_ELEMENTS)))
...
void _test_assert_array_complex_double_within(double delta, complex double* expected, complex double* actual, size_t num_elements) {
    for (size_t i = 0; i < num_elements; i++) {
...
void _test_assert_array_complex_float_within(float delta, complex float* expected, complex float* actual, size_t num_elements) {
    for (size_t i = 0; i < num_elements; i++) {
